...
module.exports = function (jsonapi, data, opts) {
  ...
  function keyForType(type) {
    if (_.isString(opts.keyForType)) {
      return opts.keyForType
    } else if (_.isFunction(opts.keyForType)) {
      return opts.keyForType(type, keyForAttribute)
    } else {
      return null;
    }
  }
  ...
  function extractAttributes(from) {
    ...
    var typeKey = keyForType(from.type);

    if (_.isString(typeKey)) {
      dest[typeKey] = from.type;
    }
